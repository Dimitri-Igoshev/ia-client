...
export const Form = ({ hasTitle = false, isModal = false, visible = false, close }: FormProps) => {
					...
					>
									...
									<div
										...
										{!file &&
											<>
												...
												<div className="ml-[15px] text-xl font-light">{t('attachment')}</div>
											</>
										}
										{file &&
											<div className="flex items-center">
												<span className="text-xl font-light">{file?.name}</span>
												<AiOutlineCloseCircle className="ml-[10px] text-xl" onClick={() => setFile(null)}/>
											</div>
										}
									</div>
									<Button size="l" type="submit">{t('button')}</Button>
								</div>
							</>
						}
						{loading &&
						...
						{!loading && !success && sent &&
							<div className="flex flex-col gap-[70px] justify-center items-start w-full h-[670px] text-xl">
								...
								<Button size="l" onClick={() => setSent(false)}>{t('resend')}</Button>
							</div>
						}
					</form>
				</Tilt>
